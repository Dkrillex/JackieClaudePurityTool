...
def render_markdown(results: list) -> str:
    ...
    out.append("- 评分维度：协议纯度(20) · 模型新鲜度(20) · 推理完整性(15) · "
               "指令忠实度(15) · 提示注入抵抗(15) · 流式完整性(10) · 凭据安全(5)")
    out.append("")
    ...
    if ref and len(results) > 1:
        ...
        out.append("## 一句话结论")
        out.append("")
        line = (f"**{ref.name}** 纯度 **{ref.total}/{ref.max_total:.0f}"
                f"（{ref.grade}）**")
        if worst:
            line += (f"，对照组最低为 **{worst.name} {worst.total}/"
                     f"{worst.max_total:.0f}（{worst.grade}）**")
        out.append(line + "。")
        out.append("")
    ...
    out.append("## 一、纯度总览")
    out.append("")
    header = ("| 渠道 | 协议 | 新鲜度 | 推理 | 指令 | 注入 | 流式 | 安全 | "
              "**总分** | 评级 |")
    out.append(header)
    out.append("|------|------|--------|------|------|------|------|------|"
               "------|------|")
    for r in sorted(results, key=lambda x: -x.total):
        ...
        out.append(
            ...
            f"{cell('reasoning')} | {cell('instruction')} | {cell('injection')} | "
            f"{cell('stream')} | {cell('leakage')} | **{r.total}** | {r.grade} |"
        )
    out.append("")
    ...
    for r in sorted(results, key=lambda x: -x.total):
        out.append("")
        out.append(f"### {r.name} — {r.total}/{r.max_total:.0f}（{r.grade}）")
        out.append("")
